...
const isAlphabet = value => {
  if (!PrimitivesIsString(value) || value.length !== 58) {
    return false
  }
...
class Base58 {
  ...
  encodeText(string, start, end) {
    return this[encodeToStringSymbol](textEncoder.encode(String(string)), start, end)
  }

  decodeText(string, start, end) {
    return textDecoder.decode(this[decodeFromStringSymbol](String(string), start, end))
  }
